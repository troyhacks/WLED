...
#else // WLED_IDF_BUILD
#include "idf_shims/AsyncWebServer.h"
#endif // WLED_IDF_BUILD
...
// ESP32-P4 IDF I2C master bus handle and port constant
#if defined(CONFIG_IDF_TARGET_ESP32P4)
#include "driver/i2c_master.h"
WLED_GLOBAL i2c_master_bus_handle_t global_i2c_bus_handle _INIT(nullptr);
#define GLOBAL_I2C_PORT I2C_NUM_0
#endif
#ifdef ARDUINO_ARCH_ESP32  // re-open the #ifdef block closed above to keep original structure
...
// PPA (Pixel Processing Accelerator) and JPEG decoder handles — ESP32-P4 only
...
// cannot do this on -S2, due to buggy USBCDC serial driver: canUseSerial
  ...
  // IDF build: no Serial class, use printf directly
  #define DEBUGOUT(x)       do { printf("%s", String(x).c_str()); } while(0)
  #define DEBUGOUTLN(x)     do { printf("%s\n", String(x).c_str()); } while(0)
  #define DEBUGOUTF(x...)   printf(x)
  #define DEBUGOUTFP(x...)  printf(x)
  #define DEBUGOUTFlush()   do {} while(0)
...
#ifdef WLED_IDF_BUILD
  #define WLED_CONNECTED (eth_is_connected || wifi_is_connected)
