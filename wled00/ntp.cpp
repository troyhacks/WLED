...
void handleNetworkTime() {
  if (ntpEnabled && ntpConnected && millis() - ntpLastSyncTime > (1000 * NTP_SYNC_INTERVAL) && WLED_CONNECTED) {
    ...
    // DEBUG_PRINTF("Not handling network time %lu vs %lu ntpEnabled: %d ntpConnected: %d WLED_CONNECTED: %d\n", millis() - ntpLastSyncTime, 1000 * NTP_SYNC_INTERVAL, ntpEnabled, ntpConnected, WLED_CONNECTED);
  }
}
