...
`;
    ...
    // `linkable: true` emits the array/length with default ELF visibility so the symbol
    // survives -fvisibility=hidden + LTO and remains reachable from external .cpp files
    // (e.g. ESPAsyncWebServer's SPIFFSEditor.cpp).
    // `__attribute__((used))` is also required to defeat `-fdata-sections -Wl,--gc-sections`,
    // which strips unreferenced data sections even when they have default visibility.
    // `extern` on the definition is REQUIRED: in C++, namespace-scope `const` variables
    // have implicit internal linkage (compiler emits `_ZL` prefix), which doesn't match
    // `extern const` declarations in consuming translation units. `extern` on the def
    // forces external linkage.
    const linkPrefix = s.linkable ? 'extern __attribute__((used, visibility("default"))) ' : '';
    const chunk = `
// Autogenerated from ${srcDir}/${s.file}, do not edit!!
${linkPrefix}const uint32_t ${s.name}_length = ${zip.length};
${linkPrefix}const uint8_t ${s.name}[] PROGMEM = {
${result}
};
...
`,
    ...
    {
      file: "edit.htm",
      name: "edit_htm_gz",   // symbol referenced by SPIFFSEditor.cpp (wled00/html_other.h overrides the version embedded in ESPAsyncWebServerWLED)
      method: "gzip",
      filter: "html-minify",
      linkable: true,        // SPIFFSEditor.cpp (in ESPAsyncWebServerWLED) lives in a separate .o; emit with default visibility so the linker can resolve it under -fvisibility=hidden + -flto
    },
    //WLEDMM
    {
      file: "peek.js",
