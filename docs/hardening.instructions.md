...
> Values that have been validated and range-clamped at ingress are **trusted** for internal WLED-MM
...
6. **No DOM XSS sinks with untrusted data** (`innerHTML`, unsafe HTML insertion). Server-side generation of JavaScript property-assignment statements (as used in WLED-MM's printSetForm* helpers) is exempt.
