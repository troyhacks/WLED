...
description: "WLEDMM-focused security review guide based on OWASP Top 10 for embedded firmware and web UI."
---

# WLED-MM Security Review Standards (Embedded + Web UI)
...
## WLED-MM Constraints and Threat Model Assumptions
...
**Untrusted data** enters WLED-MM only at the following explicit ingress points:
...
**Validation and range-clamping applied at the ingress point renders data trusted** for all subsequent use within the WLED-MM core.

**Do not flag:**
- Repeated bounds or range checks on a value that has already been validated and clamped at its ingress handler.
- Internal WLED-MM core logic that operates on values confirmed safe by the ingress layer.
...
- SQL/NoSQL checks, JWT/OAuth flows, GraphQL-specific checks, generic backend framework checks not used by WLED-MM.
...
- Skip irrelevant framework checks not used by WLED-MM.
